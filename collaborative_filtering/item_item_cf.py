...
logger = logging.getLogger("item-item-collaborative-filtering")
...
def predict_rating(
    ...
    similarity_scores = item_similarities[movie_index]       
    user_deviations = centered_user_item_matrix[user_index, :]         

    rated_mask = (user_item_matrix[user_index, :] > 0).float()  
...
if __name__ == "__main__":
    # 1. Load data
    data_frame = get_data()
    data_frame_train, data_frame_test = leave_k_out_split(data_frame, random_state=1335)
    ...
    for row in data_frame_train.itertuples():
        ...
        user_item_matrix[user_idx, movie_idx] = float(row.rating)
