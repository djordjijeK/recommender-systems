...
class NeuralCollaborativeFiltering(nn.Module):
    ...
    def __init__(
        ...
        self._gmf_item_embedding = nn.Embedding(num_movies, gmf_embedding_dim)

        # MLP branch embeddings
        self._mlp_user_embedding = nn.Embedding(num_users, mlp_embedding_dim)
        self._mlp_item_embedding = nn.Embedding(num_movies, mlp_embedding_dim)

        self._user_bias = nn.Embedding(num_users, 1)
        self._item_bias = nn.Embedding(num_movies, 1)
    ...
    def forward(self, user_ids: torch.Tensor, movie_ids: torch.Tensor) -> torch.Tensor:
        ...
        gmf_movie = self._gmf_item_embedding(movie_ids)  # [B, gmf_dim]
        gmf_out = gmf_user * gmf_movie                   # [B, gmf_dim]

        # MLP branch
        mlp_user = self._mlp_user_embedding(user_ids)      # [B, mlp_dim]
        mlp_movie = self._mlp_item_embedding(movie_ids)    # [B, mlp_dim]
        mlp_in = torch.cat([mlp_user, mlp_movie], dim=-1)  # [B, 2 * mlp_dim]
        ...
        movies_bias = self._item_bias(movie_ids).squeeze(-1)  # [B]

        return predictions + user_bias + movies_bias
